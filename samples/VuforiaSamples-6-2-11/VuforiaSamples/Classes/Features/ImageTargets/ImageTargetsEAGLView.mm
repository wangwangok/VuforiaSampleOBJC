...
//
// 中文译文
// iOS上的OpenGL ES是线程不安全的，在程序中我们使用如下方法来保证线程安全
// 1) 在主线程创建OpenGL ES上下文。
// 2) Vuforia相机开始时，将其位于我们自己EAGLView视图上，并开启render线程。
// 3) Vuforia会在render线程上，定期调用我们的``renderFrameVuforia``方法。当第一次发生的时候，``defaultFramebuffer``并不存在，所有它会被调用``createFramebuffer``方法来创建。
// createFramebuffer由主线程调用，而与此同时render线程会被阻塞。因此确保OpenGL ES上下文不会被并行使用
//
//
//
//******************************************************************************
...
namespace {
    ...
    // Model scale factor
    const float kObjectScaleNormal = 5.f;
    const float kObjectScaleOffTargetTracking = 5.f;
}
...
- (id)initWithFrame:(CGRect)frame appSession:(SampleApplicationSession *) app
{
    ...
    if (self) {
        ...
        for (int i = 0; i < kNumAugmentationTextures; ++i) {
            ...
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);/// 缩小操作，使用线性过滤
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);/// 放大操作，使用线性过滤
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, [augmentationTexture[i] width], [augmentationTexture[i] height], 0, GL_RGBA, GL_UNSIGNED_BYTE, (GLvoid*)[augmentationTexture[i] pngData]);
        }

        offTargetTrackingEnabled = kUse3DModel;
        flagc = NO;
...
- (void) renderFrameWithState:(const Vuforia::State&) state projectMatrix:(Vuforia::Matrix44F&) projectionMatrix {
    ...
    for (int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        if (offTargetTrackingEnabled) {
            glVertexAttribPointer(vertexHandle, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)pikachuModel.vertices);
            glVertexAttribPointer(normalHandle, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)pikachuModel.normals);
            glVertexAttribPointer(textureCoordHandle, 2, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)pikachuModel.texCoords);
        ...
        if (!strcmp(trackable.getName(), "chips"))
            ...
            targetIndex = 2;
        else if (!strcmp(trackable.getName(), "HXPaper"))
            targetIndex = 4;
        ...
        if (offTargetTrackingEnabled) {
            /// 绘制纹理
            glDrawArrays(GL_TRIANGLES, 0, (int)pikachuModel.numVertices);
        } else {
            /// 绘制
            /// https://stackoverflow.com/questions/16960517/replacing-teapot-in-vuforia
#if kUse3DModel == 0
